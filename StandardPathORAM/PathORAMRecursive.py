...
class PathORAMRecursive:
    ...
    def __init__(self, N) -> None:
        self.N = N # InitSize maxLevelID treeHeight
        self.Z = 2
        self.treeHeight = math.ceil(math.log2(N))
        self.maxLevelID = self.treeHeight-1 # Total self.treeHeight containing {0, 1, ..., self.maxLevelID}
        """
        Storing in Client
        """
        self.localPosMap = []
        self.posMapStash = [[] for i in range(1, self.maxLevelID)]
        self.valStash = []
        """
        Storing in Server
        """
        
        # T(1), T(2), T(3),..., T(self.maxLevelID-1)
        self.posMapTree = [[[[PathORAMRecursive.emptyPos for _ in range(self.Z)] for _ in range(2**i)] for i in range(j+1)] for j in range(1, self.maxLevelID)]
        # T(self.maxLevelID)
        self.valTree = [[[PathORAMRecursive.emptyElement for _ in range(self.Z)] for _ in range(2**i)] for i in range(self.maxLevelID+1)]
        """
        leaf:
                       0
                0              1
    ...
    def initialization(self, elementList):
        ...
        self.localPosMapStorageInKB = (len(self.localPosMap)<<1)*math.ceil(1/8)/(2**10) # save 4 positions with 2 leaves, each with 1-bit
        self.localStashStorageInKB = (16+16+math.ceil((len(self.valTree)-1)/8))*len(self.valStash) # (k, v, leafID)
        currentPosBit = 1
        for i in range(len(self.posMapStash)): # (k, nextLeafID, nextLeafID, nowLeafID)
    ...
    def access(self, k, modV, op):
        yList = [k]
        for _ in range(self.maxLevelID):
        ...
        for i in range(2,self.maxLevelID+1):
            vLoc = (yList[i]+1)%2
        ...
        for i in range(1,self.maxLevelID):
            self.posMapStash[i-1] = self.evictPosMap(self.posMapStash[i-1],self.posMapTree[i-1],posList[i-1])
        ...
        self.localPosMapStorageInKB = (len(self.localPosMap)<<1)*math.ceil(1/8)/(2**10) # save 4 locations with 2 leaves, each with 1-bit
